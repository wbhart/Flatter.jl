...
* **`size_reduction_triu.jl` blocking almost certainly isn't reaching Strassen.** At the current block size the tile products sit at or below the Strassen cutoff and go straight to the base multiplication — raise the block size to actually benefit. Blocking still pays for itself: cache effects, and one workspace grabbed up front instead of per-operation allocation. Kronecker substitution — packing a tile into one huge integer for a single GMP multiply — is NOT the answer: it turns `n^3` multiplies of `b` bits into `n^2` multiplies of `nb` bits, and since GMP's multiplication is superlinear that is worse by roughly `n^0.58`. The technique that does apply is multi-modular multiplication: reduce mod enough word-sized primes, multiply in machine integers (BLAS-backed), and CRT back, as FLINT and LinBox do.
...
* **The Arb probe says the precision policy is over-provisioned.** At `2*spread + 30 + 2n` every quotient certifies with 200-500 bits of margin, while `spread + 128` fails. So the real threshold is between them, and cutting the policy would speed up every BigFloat path at once — a broader lever than any number-type change, since the fused QR is 24-36% of most runs. `headroom()` in `lattices/arb_probe.jl` bisects for the certifying minimum.
