...
* **The fused QR is BLAS-2, and that turns out not to matter.** The trailing update applies one reflector to one column at a time, so it contains no matrix products. Blocking would route it through `_householder_block_mul!` and hence Strassen, which is what flatter's `Heuristic3` does — `Heuristic3` being flatter's SERIAL path (`Threaded3` is the threaded one) with no OpenMP at all, so its tiling is a cache and BLAS-3 optimisation rather than a parallelisation device. Measurement says the trailing update is too small a share to justify it; see the entry above.
